 ...
 * thing here that is meant to be read whole. It is what a new claim is read
 * against — whether it is already on the list gathering its second source, or
 * contradicts something standing — and a page of it, or only the lines
 * matching a term, answers that question wrongly by omission. The list is a
 * dashboard's standing knowledge rather than its coverage, so it stays in the
 * tens of lines where the story feed runs to hundreds.
 *
...
export const getFacts: AgentTool = {
  ...
  description:
    ...
    "Returns the whole list — there is no page and no search: a claim you have not read is one you cannot tell from a claim already standing. " +
    "Read it before you revise: these are the ids REVISE_FACTS takes, and naming a fact's id is what edits it in place instead of filing a duplicate beside it. An id REVISE_FACTS was not given here is refused. " +
