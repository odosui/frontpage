...
export const getStory: AgentTool = {
  ...
  description:
    "One story's articles, newest first: headline, outlet, the date it was published, how important the categorizer thought it was, its tags, and its id. Up to 50 unless you ask for a different number. " +
    "GET_STORIES and GREP_STORIES only tell you a story exists and how many articles are under it — this is how you find out what they actually say. " +
  ...
  run: async (args, ctx) => {
    ...
    const head =
      story.title +
      (story.storyline ? ` — under ${story.storyline.title}` : " — standalone");
    const shown =
